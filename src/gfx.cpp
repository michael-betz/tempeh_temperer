...
void gui(unsigned long ts_now)
{
	...
	if (heater_enabled) {
		...
		hLine(0, 1, DISPLAY_WIDTH / 2, true);
		hLine(0, 13, DISPLAY_WIDTH / 2, true);
		int16_t p = (target_heater_power + FP_ROUND) >> (FP_FRAC + 2);
		if (p > 0)
			fillRect(0, p, 3, 11, true);
	} else {
		set_cursor(1, 1);
		print_str("disabled");
	}
	...
	set_cursor(DISPLAY_WIDTH / 2 + 16, 2);
	set_size(1);
